  ...
  Future<void> _abrirRelatorio() async {
    ...
    try {
      final file = await downloadAuthenticatedFile(
        ...
        filename:
            'relatorio_proprietario_${_idProprietario}_${_inicio}_$_fim.pdf',
        query: {
            ...
            child: Column(
              children: [
                ...
                DropdownButtonFormField<String>(
                  value: _status,
                  decoration: const InputDecoration(labelText: 'Baixa'),
                  items: const [
                    DropdownMenuItem(
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () async {
                          final p =
                              await pickDateIso(context, initialIso: _inicio);
                          if (p != null) setState(() => _inicio = p);
                        },
                        icon: const Icon(Icons.event),
                        label:
                            Text('Inicio: ${AppDates.formatDateBr(_inicio)}'),
                      ),
                    ),
                  ],
                ),
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () async {
                          final p =
                              await pickDateIso(context, initialIso: _fim);
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(
                      _abrindoPdf ? 'Abrindo...' : 'Abrir PDF do relatorio'),
                ),
              ],
            ),
          ),
