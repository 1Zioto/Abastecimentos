...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _entrar() async {
    ...
    try {
      ...
      final resp = await state.api
          .post('/auth/login', {'login': login, 'password': senha});
      if (resp is! Map) {
      ...
      final expiresIn =
          resp['expires_in'] is num ? (resp['expires_in'] as num).toInt() : 0;
      ...
      final user = (resp['user'] is Map)
          ...
          : <String, dynamic>{};
      final filiais = (user['filiais_acesso'] is List)
          ? (user['filiais_acesso'] as List).map((e) => e.toString()).toList()
          : <String>[];
      ...
      if (!mounted) return;
      final filial = await _selecionarFilialInicial();
      if (filial == null) return;
      await state.auth.setFilialAtual(filial);
  ...
  Future<String?> _selecionarFilialInicial() async {
    final auth = AppState.instance.auth;
    final filiais = auth.filiaisAcesso;
    if (filiais.length <= 1) return auth.filialAtual ?? filiais.first;
    return showDialog<String>(
      context: context,
      barrierDismissible: false,
      builder: (ctx) => AlertDialog(
        title: const Text('Selecione a filial'),
        content: Column(
                ...
                (filial) => Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: FilledButton.icon(
                      onPressed: () => Navigator.pop(ctx, filial),
                      icon: Icon(filial == 'Matriz'
