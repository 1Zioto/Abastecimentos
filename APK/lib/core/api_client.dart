...
class ApiClient {
      ...
      try {
        ...
        if (resp.statusCode >= 500 && resp.statusCode < 600) {
          if (idempotent && attempt < _maxAttempts) {
            _log('$method ${uri.toString()} -> ${resp.statusCode} (retry $attempt/$_maxAttempts em ${backoff.inMilliseconds}ms)');
        ...
        if (attempt < _maxAttempts) {
          _log('$method ${uri.toString()} -> offline (retry $attempt/$_maxAttempts em ${backoff.inMilliseconds}ms): ${e.message}');
          await Future.delayed(backoff);
          backoff *= 2;
          continue;
        }
  ...
  dynamic _handle(
    ...
    final responsePreview = text.length > 1200 ? '${text.substring(0, 1200)}...[truncado]' : text;
  ...
  Future<bool> _refreshToken() async {
    ...
    try {
      ...
      final resp = await http.Response.fromStream(streamed);
      if (resp.statusCode < 200 || resp.statusCode >= 300) {
        _log('POST ${uri.toString()} -> refresh failed status=${resp.statusCode}');
        completer.complete(false);
        return false;
      }
